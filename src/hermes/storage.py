    ...
    *,
    ...
    content = "\n".join(
        [
            ...
            f"# {title}",
            "",
            "Links: [[Document Memory Candidates]] [[bairui]] [[Long Term Memory]] "
            f"[[{ingest_link}]]",
            "",
            "## Review",
            "",
            ...
            f"- Memory runtime status: {review.everos_status or 'not_called'}",
    ...
    *,
    ...
    content = "\n".join(
        [
            ...
            f"# {title}",
            "",
            "Links: [[Document Ingest Reports]] [[Document Memory Candidates]] [[bairui]] "
            "[[Document Parser]] [[Local Index]] [[Long Term Memory]] [[Knowledge Graph]]",
            "",
            "## Summary",
            "",
    ...
    moc.write_text(
        "\n".join(
            [
                ...
                "",
                "Graph links: [[bairui]] [[Long Term Memory]] [[Knowledge Graph]]",
                "",
                "This note collects reviewed document memory candidates generated by bairui.",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    moc.write_text(
        "\n".join(
            [
                ...
                "",
                "Graph links: [[bairui]] [[Document Parser]] [[Local Index]] [[Long Term Memory]] [[Knowledge Graph]]",
                "",
                "This note collects bairui document ingestion reports.",
                "",
            ]
        ),
        encoding="utf-8",
...
def _index_run_lines(index_runs: list[dict[str, Any]]) -> list[str]:
    if not index_runs:
        return ["No local index runs recorded yet."]
    lines = ["| Provider | Status | Indexed | Skipped | Failed |", "| --- | --- | ---: | ---: | ---: |"]
    for run in index_runs:
        lines.append(
            "| "
            f"{_public_provider_label(str(run.get('provider', '')))} | "
            f"{run.get('status', '')} | "
    ...
    for candidate in candidates:
        ...
        lines.extend(
            [
                ...
                f"- Memory runtime status: {review.get('everos_status', 'not_called') or 'not_called'}",
...
def _source_ref_lines(source_refs: list[dict[str, Any]]) -> list[str]:
    ...
    for ref in source_refs:
        lines.append(
            ...
            f"{_public_provider_label(str(ref.get('provider', '')))} | "
...
def _next_action_lines(
    ...
    if any(run.get("status") not in {"completed", "skipped"} for run in index_runs):
        actions.append("- Inspect failed or partial local index runs.")
    if not source_refs:
...
def _public_provider_label(provider: str) -> str:
    labels = {
