...
class RuntimeFoundationTests(unittest.TestCase):
    ...
    def test_console_static_assets_are_served_by_backend(self):
        ...
        self.assertIn(b"renderBrainUiApp", js_body)
        self.assertIn(b"/chat", js_body)
        self.assertIn(b"buildBairuiGraphRows", js_body)
        self.assertEqual(shell_status, 200)
        self.assertIn("text/javascript", shell_headers["content-type"])
        self.assertEqual(shell_headers["cache-control"], "no-store, must-revalidate")
        self.assertEqual(shell_headers["pragma"], "no-cache")
        self.assertEqual(shell_headers["expires"], "0")
    ...
    def test_console_product_errors_explain_next_steps_and_safety(self):
        ...
        self.assertIn("sanitizePublicBrandText", app_js)
    ...
    def test_command_console_composer_maps_to_agent_contracts(self):
        ...
        self.assertIn("initChat", app_js)
        self.assertIn("defaultInputPlaceholder", app_js)
        self.assertIn("onUserMessage", app_js)
        self.assertIn("toggleHotspot", app_js)
        self.assertIn("showPersonCardByName", app_js)
        self.assertIn("bairui", shell_js)
        self.assertIn("prompt: backendText", chat_js)
        self.assertIn('system: "You are bairui', chat_js)
    def test_command_console_surfaces_promotion_resource_closure(self):
        app_js = Path("web/bairui-console/app.js").read_text(encoding="utf-8")
        chat_js = Path("web/bairui-console/chat.js").read_text(encoding="utf-8")

        self.assertIn('addMsg("user"', chat_js)
        self.assertIn('addMsg("bairui"', chat_js)
        self.assertIn("external_send_performed: false", app_js)
        self.assertIn("long_term_memory_auto_write: false", app_js)
        self.assertIn("channel authorization is managed by bairui approvals", Path("src/hermes/server.py").read_text(encoding="utf-8"))

    def test_activation_console_renders_guided_steps_and_safety_boundaries(self):
        index_html = Path("web/bairui-console/index.html").read_text(encoding="utf-8")
        app_js = Path("web/bairui-console/app.js").read_text(encoding="utf-8")
        shell_js = Path("web/bairui-console/app-shell.js").read_text(encoding="utf-8")
        styles = Path("web/bairui-console/styles.css").read_text(encoding="utf-8")

        self.assertIn("bairui Console", index_html)
        self.assertIn("/console/vendor/d3/d3.min.js", index_html)
        self.assertIn("createGraphStage", shell_js)
        self.assertIn('id="graph"', shell_js)
        self.assertIn("bairui Agent", shell_js)
        ...
        self.assertIn("第四步", app_js)
        self.assertIn("核心存储", app_js)
        self.assertIn("renderActivationStorageCards", app_js)
        self.assertIn("能力模块不在首次激活里强制配置", app_js)
        ...
        self.assertIn(".activation-wizard", styles)
        self.assertIn(".activation-tabs", styles)
    ...
    def test_activation_setup_plan_exposes_first_use_config_and_runtime_commands(self):
        server_py = Path("src/hermes/server.py").read_text(encoding="utf-8")
        app_js = Path("web/bairui-console/app.js").read_text(encoding="utf-8")

        self.assertIn('if self.path == "/activation/setup-plan"', server_py)
        self.assertIn("build_activation_setup_plan", server_py)
        self.assertIn("model_base_url", server_py)
        self.assertIn("model_api_key", server_py)
        self.assertIn("required_storage", server_py)
        self.assertIn("_activation_storage_item", server_py)
        self.assertIn("build_mineru_install_command", server_py)
        self.assertIn("build_searxng_docker_command", server_py)
        self.assertIn("build_sonic_docker_command", server_py)
        self.assertIn("build_funasr_docker_command", server_py)
        self.assertIn("build_trendradar_mcp_command", server_py)
        self.assertIn("saveActivationModelConfig", app_js)
        self.assertIn("打开完整设置", app_js)
        self.assertIn("settings-save-core-config", app_js)
        self.assertIn("settings-activation-plan-grid", app_js)
        self.assertIn("saveOwnerAuthToken", Path("web/bairui-console/api-client.js").read_text(encoding="utf-8"))

    def test_documents_console_maps_next_actions_to_real_backend_contracts(self):
        app_js = Path("web/bairui-console/app.js").read_text(encoding="utf-8")
        doc_js = Path("web/bairui-console/doc.js").read_text(encoding="utf-8")
        server_py = Path("src/hermes/server.py").read_text(encoding="utf-8")
    ...
    def test_memory_review_console_keeps_owner_review_and_source_trace_visible(self):
        ...
        self.assertIn("agent:bairui", app_js)
    ...
    def test_reports_console_surfaces_detail_sources_and_write_result(self):
        ...
        self.assertIn("source:reference", app_js)
        self.assertIn("可追踪来源", app_js)
    ...
    def test_codegraph_console_keeps_source_structure_separate_from_memory(self):
        ...
        self.assertIn("/codegraph/status", status_paths)
        self.assertIn("CodeGraph", Path("web/bairui-console/app-shell.js").read_text(encoding="utf-8") + app_js)
        self.assertIn("source-structure indexes separately from long-term memory", Path("src/hermes/config_status.py").read_text(encoding="utf-8"))
    ...
    def test_avatar_console_surfaces_browser_runtime_and_approval_boundary(self):
        ...
        self.assertIn("/avatar/status", status_paths)
        self.assertIn("/avatar/manifest", status_paths)
    ...
    def test_events_console_surfaces_audit_timeline_and_safety_evidence(self):
        ...
        self.assertIn("connectSSE", app_js)
        self.assertIn("new EventSource", app_js)
        self.assertIn("/events", app_js)
        self.assertIn("/audit/stats", server_py)
        self.assertIn("memory_recall_rate", server_py)
        self.assertIn("audit:event", app_js)
    def test_entity_cards_are_typed_traceable_product_objects(self):
        app_js = Path("web/bairui-console/app.js").read_text(encoding="utf-8")
        person_js = Path("web/bairui-console/person-card.js").read_text(encoding="utf-8")
        shell_js = Path("web/bairui-console/app-shell.js").read_text(encoding="utf-8")

        self.assertIn("initPersonCard", app_js)
        self.assertIn("showPersonCardByName", app_js)
        self.assertIn("createPersonCardPanel", shell_js)
        self.assertIn("person-card", person_js)
        self.assertIn("entity", person_js.lower())
    ...
    def test_commercial_go_no_go_collects_final_trial_gates(self):
        ...
        wecom_trial_bash = Path("scripts/run-wecom-channel-trial.sh").read_text(encoding="utf-8")
        readme = Path("README.md").read_text(encoding="utf-8")
        handoff = Path("docs/29-commercial-trial-handoff-pack.md").read_text(encoding="utf-8")
        ...
        self.assertIn("run-wecom-channel-trial.sh", readme)
        self.assertIn("run-wecom-channel-trial.sh", handoff)
        self.assertIn("run-wecom-channel-trial.sh", report)
        ...
        self.assertIn("BOT_KEY", wecom_trial_bash)
        self.assertIn("BAIRUI_WECOM_TRIAL_BOT_KEY", wecom_trial_bash)
        self.assertIn("apply_local_config", wecom_trial_bash)
        self.assertIn("DANGEROUS_CONFIRMATION_PHRASE", wecom_trial_bash)
        self.assertIn("channels wecom-trial", wecom_trial_bash)
        self.assertIn("--approve", wecom_trial_bash)
        self.assertIn("artifacts/wecom-trial.json", wecom_trial_bash)
        self.assertIn("artifacts/wecom-receipt.json", wecom_trial_bash)
        self.assertIn("secret_echo", wecom_trial_bash)
        self.assertIn("unset BAIRUI_WECOM_TRIAL_BOT_KEY", wecom_trial_bash)
        self.assertNotIn("WECOM_BOT_KEY=", wecom_trial_bash)
        ...
        self.assertIn("renderBrainUiApp", script)
        self.assertIn("buildBairuiGraphRows", script)
        self.assertIn("ownerAuthHeaders", script)
        self.assertIn("ENTITY_CARD_KIND", script)
