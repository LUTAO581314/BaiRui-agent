...
class RuntimeFoundationTests(unittest.TestCase):
    ...
    def test_console_static_assets_are_served_by_backend(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict(os.environ, env, clear=False):
                ...
                try:
                    ...
                    three_status, three_headers, three_body = _http_get(server.server_port, "/console/vendor/three.module.js")
                    three_core_status, three_core_headers, three_core_body = _http_get(server.server_port, "/console/vendor/three.core.js")
        ...
        self.assertIn("text/javascript", three_headers["content-type"])
        self.assertIn(b"export {", three_body)
        self.assertEqual(three_core_status, 200)
        self.assertIn("text/javascript", three_core_headers["content-type"])
        self.assertIn(b"WebGLRenderer", three_core_body)
    ...
    def test_activation_console_renders_guided_steps_and_safety_boundaries(self):
        ...
        self.assertIn("function activationDeploymentModes", app_js)
        self.assertIn("function selectedActivationMode", app_js)
        self.assertIn("function renderActivationModeSelector", app_js)
        self.assertIn("function renderActivationModeNextStep", app_js)
        self.assertIn("function renderActivationCoreStage", app_js)
        self.assertIn("function renderActivationCoreLayer", app_js)
        self.assertIn("function activationCoreSteps", app_js)
        self.assertIn("loading_contract", app_js)
        self.assertIn("loading_runtime", app_js)
        self.assertIn('stepId.startsWith("loading")', app_js)
        self.assertIn("import * as THREE", app_js)
        self.assertIn("function initActivationThreeCore", app_js)
        self.assertIn("function disposeActivationThreeCore", app_js)
        self.assertIn("new THREE.Scene", app_js)
        self.assertIn("new THREE.WebGLRenderer", app_js)
        self.assertIn("preserveDrawingBuffer", app_js)
        self.assertIn("renderer.shadowMap.enabled = true", app_js)
        self.assertIn("new THREE.FogExp2", app_js)
        self.assertIn("new THREE.GridHelper", app_js)
        self.assertIn("new THREE.Raycaster", app_js)
        self.assertIn("new THREE.Points", app_js)
        self.assertIn("new THREE.BufferGeometry", app_js)
        self.assertIn("new THREE.TubeGeometry", app_js)
        self.assertIn("new THREE.CatmullRomCurve3", app_js)
        self.assertIn("new THREE.MeshPhysicalMaterial", app_js)
        self.assertIn("new THREE.CanvasTexture", app_js)
        self.assertIn("function createActivationGlowSprite", app_js)
        self.assertIn("function createActivationFresnelMaterial", app_js)
        self.assertIn("function createActivationBokehField", app_js)
        self.assertIn("function createActivationThreeLabel", app_js)
        self.assertIn("function clampActivationLabel", app_js)
        self.assertIn("function roundRect", app_js)
        self.assertIn("activeEnergyPaths", app_js)
        self.assertIn("particleField", app_js)
        self.assertIn("glassShell", app_js)
        self.assertIn("lensAura", app_js)
        self.assertIn("cameraRig", app_js)
        self.assertIn("verticalBeam", app_js)
        self.assertIn("scanPlane", app_js)
        self.assertIn("fresnelCore", app_js)
        self.assertIn("apertureGroup", app_js)
        self.assertIn("starGate", app_js)
        self.assertIn("bokehField", app_js)
        self.assertIn("new THREE.ShaderMaterial", app_js)
        self.assertIn("new THREE.WebGLRenderTarget", app_js)
        self.assertIn("new THREE.OrthographicCamera", app_js)
        self.assertIn("new THREE.PlaneGeometry(2, 2)", app_js)
        self.assertIn("function createActivationPostMaterial", app_js)
        self.assertIn("postMaterial.uniforms.tDiffuse.value = renderTarget.texture", app_js)
        self.assertIn("renderer.setRenderTarget(renderTarget)", app_js)
        self.assertIn("renderer.render(postScene, postCamera)", app_js)
        self.assertIn("uBoost", app_js)
        self.assertIn("aberration", app_js)
        self.assertIn("vignette", app_js)
        self.assertIn("grain", app_js)
        self.assertIn("fragmentShader", app_js)
        self.assertIn("fresnel", app_js)
        self.assertIn("camera.lookAt(cameraRig.focus)", app_js)
        self.assertIn("drag orbit", app_js)
        self.assertIn('canvas.addEventListener("pointerdown"', app_js)
        self.assertIn('canvas.addEventListener("pointerup"', app_js)
        self.assertIn('canvas.addEventListener("wheel"', app_js)
        self.assertIn("target.zoom", app_js)
        self.assertIn("baseRotationY", app_js)
        self.assertIn("pickables.push", app_js)
        self.assertIn("ResizeObserver", app_js)
        self.assertIn("function activationCoreRuntimeProfile", app_js)
        ...
        self.assertIn("bairui 激活", app_js)
        self.assertIn("先选择激活方式", app_js)
        self.assertIn("服务器 / 域名生产环境", app_js)
        self.assertIn("本地生产环境", app_js)
        self.assertIn("本地试用 / 演示", app_js)
        self.assertIn("进入设置后优先填写模型网关、PostgreSQL、数据目录、渠道目标和 owner token。", app_js)
        self.assertIn("data-activation-mode", app_js)
        self.assertIn('activationMode: "server_production"', app_js)
        self.assertIn('state.contract?.brand?.name === "bairui"', app_js)
        self.assertIn("state.contract?.visibility_policy?.public_brand", app_js)
        self.assertIn("profile?.display_name", app_js)
        self.assertIn("profile?.display_detail", app_js)
        self.assertIn("data-activation-three", app_js)
        self.assertIn('data-runtime="${escapeHtml(profile?.name || "")}"', app_js)
        self.assertIn('document_runtime: "mineru_document_parse"', app_js)
        self.assertIn('codegraph: "bairui_codegraph"', app_js)
        ...
        self.assertIn(".activation-mode-panel", styles)
        self.assertIn(".activation-mode-grid", styles)
        self.assertIn(".activation-mode-card.active", styles)
        self.assertIn(".activation-mode-next", styles)
        self.assertIn(".activation-core-stage", styles)
        self.assertIn(".activation-three-canvas", styles)
        self.assertIn(".activation-three-overlay", styles)
        self.assertIn(".activation-cinema-hint", styles)
        self.assertIn(".activation-core-viewport::after", styles)
        self.assertIn("repeating-linear-gradient", styles)
        self.assertIn(".activation-core-viewport.is-picking-core", styles)
        self.assertIn("cursor: grab", styles)
        self.assertIn("cursor: grabbing", styles)
        self.assertIn(".activation-core-layer-copy em", styles)
    ...
    def test_documents_console_maps_next_actions_to_real_backend_contracts(self):
        ...
        self.assertIn("function renderDocumentIngestPackSummary", app_js)
        self.assertIn("async function exportDocumentIngestPack", app_js)
        self.assertIn("await Promise.all([loadReports(), loadMemory()])", app_js)
        self.assertIn('data-document-command="${escapeHtml(action.command || "")}"', app_js)
        self.assertIn('id="export-document-ingest-pack"', app_js)
        self.assertIn('exportConsoleData("document-ingest-pack"', app_js)
        self.assertIn("document_ingest_evidence", app_js)
        self.assertIn("audit_evidence", app_js)
        self.assertIn("includes_local_file_contents: false", app_js)
        self.assertIn("external_send_performed: false", app_js)
        self.assertIn("long_term_memory_auto_write: false", app_js)
        self.assertIn("changes_runtime_state: false", app_js)
    ...
    def test_memory_review_console_keeps_owner_review_and_source_trace_visible(self):
        ...
        self.assertIn("function renderMemoryQueueEvidenceSummary", app_js)
        self.assertIn("function renderMemorySafetyPanel", app_js)
        self.assertIn("function renderMemoryReviewCommandCenter", app_js)
        self.assertIn("function exportSelectedMemoryCandidate", app_js)
        self.assertIn("function exportMemoryData", app_js)
        self.assertIn("function memoryReviewsForCandidate", app_js)
        ...
        self.assertIn("Review candidates one by one, export evidence, and keep long-term memory writes behind explicit owner approval.", app_js)
        self.assertIn('exportConsoleData("memory-candidate"', app_js)
        self.assertIn('exportConsoleData("memory-review"', app_js)
        self.assertIn("memory_review_queue_evidence", app_js)
        self.assertIn("long_term_memory_auto_write: false", app_js)
        self.assertIn("requires_owner_review: true", app_js)
        self.assertIn("batch_reject_only: true", app_js)
        self.assertIn("batch approve path", app_js)
        ...
        self.assertIn('data-memory-export="${escapeHtml(candidate.id)}"', app_js)
        ...
        self.assertIn(".memory-queue-evidence", styles)
        self.assertIn(".memory-command-center", styles)
        self.assertIn(".memory-command-grid", styles)
    ...
    def test_reports_console_surfaces_detail_sources_and_write_result(self):
        ...
        self.assertIn("function renderReportDeliveryEvidence", app_js)
        self.assertIn("function reportAuditEvidence", app_js)
        self.assertIn("function exportSelectedReportDelivery", app_js)
        self.assertIn("function hydrateSelectedEntity", app_js)
        ...
        self.assertIn('data-report-export="${escapeHtml(report.id || report.path || "")}"', app_js)
        ...
        self.assertIn('exportConsoleData("report-delivery"', app_js)
        self.assertIn("includes_local_file_contents: false", app_js)
        self.assertIn("related_source_refs", app_js)
        self.assertIn("audit_evidence", app_js)
        ...
        self.assertIn(".report-delivery-evidence", styles)
    ...
    def test_channels_console_keeps_approval_and_no_send_boundary_visible(self):
        ...
        self.assertIn("function renderChannelApprovalCommandCenter", app_js)
        ...
        self.assertIn("function channelReviewForApproval", app_js)
        self.assertIn("function exportSelectedChannelApproval", app_js)
        self.assertIn("function renderChannelMediaBoundary", app_js)
        self.assertIn("function exportChannelQueueEvidence", app_js)
        self.assertIn('api.post("/channels/send"', app_js)
        self.assertIn('api.post("/channels/approvals/review"', app_js)
        self.assertIn('exportConsoleData("channel-approval"', app_js)
        self.assertIn('exportConsoleData("channel-approval-queue"', app_js)
        self.assertIn("channel_approval_queue_evidence", app_js)
        self.assertIn('id="export-channel-approval"', app_js)
        self.assertIn('id="export-channel-queue"', app_js)
        self.assertIn('data-channel-export="${escapeHtml(item.id)}"', app_js)
        self.assertIn("supported_media_kinds", app_js)
        self.assertIn("attachment_required_for", app_js)
        self.assertIn("includes_attachment_contents: false", app_js)
        self.assertIn("approval_bypass_allowed: false", app_js)
        self.assertIn("will_send=false", app_js)
        self.assertIn("Plan outbound drafts, record owner decisions, export evidence, and keep dispatch disabled in this product stage.", app_js)
        self.assertIn("Approval exports omit secrets and include audit-safe no-send evidence.", app_js)
        ...
        self.assertIn("external_send_performed: false", app_js)
        self.assertIn("requires_owner_review: true", app_js)
        self.assertIn(".channel-command-center", styles)
        self.assertIn(".channel-command-grid", styles)
        self.assertIn(".channel-safety-grid", styles)
        self.assertIn(".channel-media-boundary", styles)
    ...
    def test_settings_console_surfaces_runtime_readiness_and_safe_repair_actions(self):
        ...
        self.assertIn("function renderSettingsActivationProfile", app_js)
        self.assertIn("function settingsActivationRequiredFields", app_js)
        self.assertIn("function settingsActivationRestartFields", app_js)
        self.assertIn("function settingsActivationModeInstruction", app_js)
        self.assertIn("function settingsActivationFieldReady", app_js)
        self.assertIn("function renderSettingsActivationField", app_js)
        self.assertIn("function renderSettingsConfigApplyVerification", app_js)
        self.assertIn("async function verifySettingsConfigApply", app_js)
        self.assertIn("function renderSettingsSecurityAcceptance", app_js)
        self.assertIn("function settingsSecurityAcceptanceItems", app_js)
        self.assertIn("function renderSettingsDeliveryCheckEntry", app_js)
        self.assertIn("function renderSettingsConfigActionGuide", app_js)
        self.assertIn("function renderSettingsSelfServiceClosure", app_js)
        self.assertIn("function settingsSelfServiceNextActions", app_js)
        ...
        self.assertIn("function settingsSafeDraft", app_js)
        ...
        self.assertIn("Security boundary checklist", app_js)
        self.assertIn("Delivery check entry", app_js)
        self.assertIn("export-commercial-handoff-bundle.ps1 -IncludeDocs", app_js)
        self.assertIn("server-deployment-verification.json", app_js)
        self.assertIn("postgres-production-verification.json", app_js)
        self.assertIn("commercial-handoff-bundle/manifest.json", app_js)
        self.assertIn("Commercial trial controls are shown as verifiable product evidence", app_js)
        self.assertIn("Owner-gated writes", app_js)
        self.assertIn("Secrets never echo", app_js)
        self.assertIn("Scoped local paths", app_js)
        self.assertIn("Dangerous change confirmation", app_js)
        self.assertIn("Approvals cannot be bypassed", app_js)
        self.assertIn("Redacted support exports", app_js)
        self.assertIn("激活方式配置画像", app_js)
        self.assertIn("这里把第一步选择转换成客户可执行的配置清单。", app_js)
        self.assertIn("服务器生产模式必须补齐数据库、管理员令牌、数据目录、渠道审批和运行时健康检查", app_js)
        self.assertIn("试用模式先跑通最小模型、文档、记忆审核和 Avatar", app_js)
        self.assertIn('if (modeId === "local_trial") return [...common, "memory_vault_dir", "avatar_assets_dir"];', app_js)
        self.assertIn('if (modeId === "server_production") fields.push("database_url", "owner_token", "channel_targets_json");', app_js)
        self.assertIn('settingsConfigSecretState("model_gateway", "api_key") === "configured"', app_js)
        ...
        self.assertIn('Configured; enter a new key only to rotate', app_js)
        self.assertIn('Configured; enter a new URL only to rotate', app_js)
        self.assertIn('Configured; enter a new token only to rotate', app_js)
        self.assertIn("non-secret fields from /config/status", app_js)
        self.assertIn("settingsSafeDraft(values)", app_js)
        self.assertIn('"database_url"', app_js)
        ...
        self.assertIn("next_step=", app_js)
        self.assertIn("configApplyVerification", app_js)
        self.assertIn("Auto verification", app_js)
        self.assertIn("Save completed and runtime diagnostics were refreshed automatically.", app_js)
        self.assertIn("missing_required not loaded", app_js)
        self.assertIn("Restart the bairui server after saving", app_js)
        self.assertIn("Save outcome guide", app_js)
        self.assertIn("trial customers do not guess about restart or next steps", app_js)
        self.assertIn("Self-service apply closure", app_js)
        self.assertIn("Secret values show configured only", app_js)
        self.assertIn("Type APPLY BAIRUI CONFIG", app_js)
        self.assertIn("Restart the bairui process so environment-backed settings are reloaded.", app_js)
        self.assertIn("Model API, data paths, memory vault, Avatar assets, CodeGraph root, and channel targets can be entered here.", app_js)
        self.assertIn("Use Events to export a redacted diagnostic bundle if a customer issue remains.", app_js)
        self.assertIn(".settings-self-service-closure", styles)
        self.assertIn(".settings-closure-grid", styles)
        self.assertIn(".settings-closure-next", styles)
        self.assertIn("Runtime action", app_js)
        self.assertIn("re-enter any secret values before saving", app_js)
        ...
        self.assertIn(".settings-apply-verification", styles)
        self.assertIn(".settings-action-guide", styles)
        self.assertIn(".settings-action-guide-grid", styles)
        ...
        self.assertIn(".settings-activation-profile", styles)
        self.assertIn(".settings-activation-grid", styles)
        self.assertIn(".settings-activation-field-grid", styles)
        self.assertIn(".settings-activation-field-grid div.missing", styles)
        self.assertIn(".settings-security-acceptance", styles)
        self.assertIn(".settings-security-grid", styles)
        self.assertIn(".settings-delivery-check", styles)
        self.assertIn(".settings-delivery-grid", styles)
    ...
    def test_events_console_surfaces_audit_timeline_and_safety_evidence(self):
        ...
        self.assertIn('api.get("/errors")', app_js)
        self.assertIn('id="load-metrics"', app_js)
        self.assertIn('id="load-errors"', app_js)
        self.assertIn('id="export-diagnostics"', app_js)
        self.assertIn('id="export-handoff-pack"', app_js)
        self.assertIn("function loadMetrics", app_js)
        self.assertIn("function loadErrorLogs", app_js)
        self.assertIn("function renderMetricsSummary", app_js)
        self.assertIn("function renderErrorLogSummary", app_js)
        ...
        self.assertIn("memory_reviews", app_js)
        self.assertIn("channel_approvals", app_js)
        self.assertIn("source_refs", app_js)
        self.assertIn("safety_acceptance", app_js)
        self.assertIn("Metrics not loaded yet.", app_js)
        self.assertIn("Error logs not loaded yet.", app_js)
        self.assertIn("Recent redacted errors", app_js)
        self.assertIn("Event export combines durable audit records, recent redacted error logs", app_js)
        ...
        self.assertIn(".handoff-pack-strip", styles)
        self.assertIn(".event-command-center", styles)
        self.assertIn(".event-safety-grid", styles)
        self.assertIn(".error-log-summary", styles)
        self.assertIn(".error-log-list", styles)
    ...
    def test_entity_cards_are_typed_traceable_product_objects(self):
        ...
        self.assertIn("function resourceTypeForEntityType", app_js)
        self.assertIn("function hydrateSelectedEntity", app_js)
        self.assertIn("source: state.sourceRefs.map", app_js)
        self.assertIn("audit: state.audit.map", app_js)
        self.assertIn("hydrateSelectedEntity();", app_js)
    ...
    def test_product_acceptance_script_documents_demo_scenarios_and_safety_gates(self):
        ...
        self.assertIn("python -m src.hermes admin-session", script)
        ...
        self.assertIn("owner_admin_gate", script)
        self.assertIn("no_external_send", script)
        self.assertIn("no_auto_memory_write", script)
        self.assertIn("owner_token_not_echoed", script)
        ...
        self.assertIn("Local owner admin gate", doc)
    ...
    def test_commercial_trial_quickstart_covers_activation_observability_and_brand(self):
        ...
        self.assertIn("python -m src.hermes admin-session", doc)
    ...
    def test_server_trial_acceptance_runner_chains_target_server_evidence(self):
        ...
        self.assertIn("deploy-usable.ps1", script)
        self.assertIn("verify-server-deployment.ps1", script)
        self.assertIn("verify-postgres-production.ps1", script)
        self.assertIn("commercial-go-no-go.ps1", script)
        self.assertIn("export-commercial-handoff-bundle.ps1", script)
        self.assertIn("SkipDeploy", script)
        self.assertIn("SkipServerVerification", script)
        self.assertIn("SkipPostgres", script)
