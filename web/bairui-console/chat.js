...
import { ownerAuthHeaders } from "./api-client.js";
...
export function initChat({
  ...
  let closeTimer = null;
  let hasPendingbairuiMessage = false;
  let pendingMessageDismissed = false;
  let liveEl = null;  // 正在流式输出的 bairui 气泡（边收 token 边重渲染），message 事件到达后定稿
  let audioCtx = null;
  ...
  async function playbairuiAlert() {
    const ctx = ensureAudioContext();
    if (!ctx) return;
  ...
  async function fetchChatHistory() {
    try {
      const res = await fetch(`${apiBase}/agents/events?limit=${maxHistory}`, { cache: "no-store", headers: ownerAuthHeaders() });
      if (!res.ok) return [];
      const body = await res.json();
      const rows = body.agent_events || body.conversations || [];
      if (!Array.isArray(rows)) return [];
      return rows
        .filter(r => r && (r.role === "user" || r.role === "bairui" || r.role === "assistant") && typeof (r.content || r.prompt || r.detail) === "string")
        .map(r => {
          ...
          return { role: r.role === "assistant" ? "bairui" : r.role, text: r.content || r.prompt || r.detail };
        });
    } catch { return []; }
  }
  ...
  function openChat(autoClose = false) {
    ...
    if (autoClose && (!hasPendingbairuiMessage || pendingMessageDismissed) && !isTyping()) scheduleClose(4500);
  }

  function closeChat() {
    if ((hasPendingbairuiMessage && !pendingMessageDismissed) || isTyping() || isHoveringChat()) return;
  ...
  function addMsg(role, text, options = {}) {
    const { alert = role === "bairui", pending = true, label } = options;
    const defaultLabel = role === "user" ? "You" : role === "bairui" ? getAgentName() : "Peer";
    ...
    if (role === "bairui") {
      hasPendingbairuiMessage = pending;
      pendingMessageDismissed = !pending;
      if (alert) playbairuiAlert();
      if (pending) openChat();
    } else if (role === "user") {
      hasPendingbairuiMessage = false;
  ...
  msgInput.addEventListener("input", () => {
    ...
    else if (!hasPendingbairuiMessage || pendingMessageDismissed) scheduleClose();
  });
  ...
  // ── 流式回复气泡 ───────────────────────────────────────────────
  // 后端 LLM 边生成边通过 stream_chunk 推 token；这里先建一个空的 bairui 气泡，
  // 随 token 到达不断重渲染，等权威的 message 事件到达再 finalize 成最终干净全文。
  // 该气泡始终是最后一个 .msg-bairui，所以打断 ✋（updateLastbairuiMsg）照常作用其上。
  function beginLivebairuiMsg({ alert = true } = {}) {
    if (liveEl) finalizeLivebairuiMsg(null);  // 兜底：上一轮孤儿气泡先定稿
    const div = document.createElement("div");
    div.className = "msg msg-bairui msg-live";
    const labelSpan = document.createElement("span");
    labelSpan.className = "msg-label";
  ...
  function updateLivebairuiMsg(text) {
    if (!liveEl) return;
  ...
  function finalizeLivebairuiMsg(text) {
  ...
  function hasLivebairuiMsg() { return !!liveEl; }

  function updateLastbairuiMsg(newText) {
    const msgs = chatMessages.querySelectorAll('.msg-bairui');
    if (!msgs.length) return;
    const last = msgs[msgs.length - 1];
