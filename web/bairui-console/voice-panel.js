...
// 暴露 initVoicePanel + window.bairuiVoice（承重墙：app.js 的 TTS 打断与视频/音乐联动依赖它）。
//
// 解耦结构：
...
export function initVoicePanel({
  ...
  // ─── 承重墙：window.bairuiVoice 接口契约（app.js 依赖，不可改形状） ───
  window.bairuiVoice = {
  ...
  window.addEventListener('bairui:video-mode', (event) => {
    if (event.detail?.active) {
      window.bairuiVoice.suspendForMedia();
    } else {
      window.bairuiVoice.resumeAfterMedia();
    }
  });

  window.addEventListener('bairui:music-mode', (event) => {
    if (event.detail?.active) {
      window.bairuiVoice.suspendForMedia();
    } else {
      window.bairuiVoice.resumeAfterMedia();
