...
async function findPersonImage(name = '') {
  ...
  if (!query || query === '实体卡片' || query === '未知对象') return '';
  const endpoints = [
...
function renderPersonCard(card = {}) {
  ...
  setText('pc-title', card.title || '实体卡片');
  setText('pc-summary', card.summary || '暂无简介。');
  setText('pc-source', `来源：${card.source || '实体卡片'}`);
...
export async function showPersonCardByName(name, { source = 'brain-ui' } = {}) {
  ...
  try {
    ...
    console.warn('[PersonCard] 实体卡片加载失败:', err.message);
    setPersonCardMode(true, {
      source,
      card: {
        name: query,
        title: '实体卡片',
        summary: '暂时没有资料。可以让 bairui 补充这个对象的身份、来源和关联记录。',
        knownFor: [],
        tags: ['待补充'],
        source: 'fallback',
        updatedAt: new Date().toISOString(),
...
export function initPersonCard() {
  renderPersonCard(currentCard || {
    name: '实体卡片',
    title: '待命',
    summary: '选择知识图谱节点后，bairui 会在这里显示对象摘要、来源、关联记录和可执行动作。',
    knownFor: [],
    tags: ['standby'],
    source: 'standby',
  });
