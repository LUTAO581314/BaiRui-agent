...
const FX_STORAGE_KEY = 'bairui.ttsfx.v2'      // 音效参数（手感）；v2＝金属感默认调高+新增金属/Flanger参数
const FX_VOICES_KEY = 'bairui.ttsfx.voices'   // 哪些音色开启了音效（音色 ID 列表）
...
export function setbairuiFxParams(patch = {}) {
  const next = { ...readParams(), ...patch }
  try { localStorage.setItem(FX_STORAGE_KEY, JSON.stringify(next)) } catch { /* ignore */ }
  return next
}

export function getbairuiFxParams() {
  return readParams()
}

// 清除自定义参数，恢复默认档
export function resetbairuiFxParams() {
...
const UNLOCK_KEY = 'bairui.ttsfx.unlocked'
...
export function attachbairuiFx(audioEl, voiceId) {
